...
describe('isWithinAttackRange', () => {
  it('直接攻撃(射程1)は隣接マスのみ射程内', () => {
    const units = makeUnits([{ col: 2, row: 2, unitType: 'mediumTank', army: 'player' }]);
...
describe('findAttackableTargets', () => {
  it('射程内の敵ユニットのみを対象にする', () => {
    const units = makeUnits([
      { col: 2, row: 2, unitType: 'mediumTank', army: 'player' },
  ...
  it('射程内に敵がいなければ空配列を返す', () => {
    const units = makeUnits([
      ...
      { col: 5, row: 5, unitType: 'mediumTank', army: 'enemy' },
    ]);
