...
export const MAP_LIST: readonly ResolvedMapEntry[] = (
  [
    ...
    },
    {
      ...
      description:
        '「日」を横倒しにした街道が四隅を結ぶ 13x11 の地上戦マップ。両軍とも本拠地 1 + 工場 3・収入 4000・初期資金 0 の対等な立ち上がりで、左下が先手の自軍・右上が後手の敵軍の陣地。街道に挟まれた内側は左が山(車両は進入不可)・中央が平地・右が森(装輪車両は進入不可)の 3 帯に分かれる。盤面中央の中立研究所は両軍の前線工場から等距離(歩兵でちょうど 3 ターン)で、先に動ける自軍が新型戦車を手にする。中立都市は 14 個あり、先に届くのは自軍 6 個・敵軍 8 個で後手が 2 個ぶん取りやすい。',
