...
export const MAP_LIST: readonly ResolvedMapEntry[] = (
  [
    ...
    },
    {
      ...
      description:
        '海に囲まれた 4 つの島が並ぶ 35x17 の横長マップ。左が先手の自軍・右が後手の敵軍の細身の島で、真ん中の上下に中立の空港 1 個と中立都市 3 個を持つ小島が 1 つずつ浮かぶ(北は横長の砂州・南は小さな丸い岩島で、形も都市の並びも別物)。港が 1 つも無いので島を渡れるのは飛行ユニットだけで、歩兵を運べるのは空港で作る輸送ヘリだけ。道路も無く陸地は平地と森だけなので、歩兵以外の地上ユニットは自分の島から出られない守備専用になる。両軍とも本拠地 1・工場 2・空港 2(収入 5000)・初期資金 0 の対等な立ち上がりで、5 拠点は本拠地から 2 マス以内に固まっている。空港から担当する中立島までは北も南も 11 マスで、島のどのマスから飛んでも輸送ヘリで 2 ターンかかる。島の端の中立都市 5 個はどれも陣地から歩兵 1 ターン(3 マス)で、後手の敵軍は本拠地の隣に中立都市が 1 個多い。',
    },
    {
      id: 'ringLaboratory',
    ...
    {
      id: 'lakeRingRoad',
      definition: LAKE_RING_ROAD_MAP,
      description:
        '中央の湖を囲む陸地と街道が「G」の字を描く 28x24 のマップ。島の上の真ん中が先手の自軍(本拠地 1 + 工場 3 + 都市 3・収入 7000)、「G」の横棒の先っちょが後手の敵軍(本拠地 1 + 工場 3・収入 4000)で、初期資金は両軍とも 0。敵軍の本拠地は三方を湖に守られており、攻めるにも守るにも道は横棒 1 本だけ。「G」の切れ目にあたる右上では街道が途切れ、輪をつないでいるのは幅 1〜2 マスの山と森の峠だけになる。この峠が両軍を最短で結ぶ道で、敵本拠地までは歩兵 25・装軌車両 31 だが、森にも山にも入れない装輪車両は街道を大きく遠回りして 64。峠の真ん中には中立の研究所が 1 個あるきりで、平地へ抜けるまで都市は 1 つも無い。湖の水は左下の隅から幅 1 マスの川になって西の海へ抜け、街道が川を横切る (4,18) だけが橋になる。峠も川も通れない装輪車両にとっては、この橋が盤面の南北をつなぐ唯一の道になる。橋の少し左上の北岸にもう 1 個の中立研究所がある。中立拠点は 29 個(都市 27・研究所 2)で、先に届くのは自軍 12 個・敵軍 17 個。とくに「G」の右下には中立都市 10 個が固まっており、こちらは敵軍のほうが近い。',
      // 研究所と、湖・川・橋と峠の隘路を主題にした新マップ
      group: 'new',
    },
    {
      id: 'arcIslandRoad',
      definition: ARC_ISLAND_ROAD_MAP,
      description:
        '「C」を左右反転した形の島を街道が弧なりに走る 30x24 のマップ。下の腕の左端が先手の自軍・上の腕の左端が後手の敵軍で、両軍とも本拠地 1 + 隣接する工場 3 + 1 マス離れた港 2 + 1 マス離れた空港 1(収入 7000)・初期資金 0 の対等な立ち上がり。左が開いた弧なので湾ごしの直線は 13 マスしかないのに、陸路は弧をぐるりと回って歩兵 43・装軌車両 52 マス。湾を突っ切る海路は港から港まで 9(輸送艦で 2 ターン)、空路は空港から敵本拠地まで 15(戦闘機で 2 ターン)で、勝負は「目の前の湾を渡る/渡らせない」に寄る。街道は弧に沿って両陣地から伸びるが、島の右側の真ん中だけは 4 列の森・山地帯で途切れる。ここを越えられるのは歩兵と装軌車両だけで、森にも山にも入れない装輪車両は輸送艦に乗るしか相手側へ回る道が無い。自軍の街道の先、森・山地帯へ入る直前には中立の研究所 2 個・都市 3 個・空港 1 個・港 1 個が固まっており、研究所はどちらも自軍のほうが近い。陣地の少し左上の湾口にも中立港が 1 個ある。中立拠点は 38 個(都市 33・研究所 2・港 2・空港 1)で、先に届くのは自軍 17 個・敵軍 21 個。',
